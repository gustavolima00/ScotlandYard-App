    ...
    get_case = async () =>{
        ...
        .then (function (response) {
            ...
            if(response.status>= 200 && response.status<300){
                self.setState({titulo:response.data.titulo});
                self.setState({descricao:response.data.descricao});
            }
        })
      ...
      return (
        <View style={[ container.backgroud_2, { justifyContent:'space-between' }]}>
            ...
            <View style={{marginTop:20, marginLeft:20, marginRight:20}}>
                <Text style={text.case_title}> {this.state.titulo} </Text>
                <View style={container.line_2}/>
            </View>
            <ScrollView style={{marginBottom:20, marginLeft:20, marginRight:20}}>
                <Text style={text.case_text}> {this.state.descricao} </Text>
            </ScrollView>
        </View>
      )
  }
